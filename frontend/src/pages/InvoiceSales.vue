...
import { FileText, CheckCircle, Clock, X, ExternalLink, ChevronLeft, ChevronRight } from 'lucide-vue-next'
...
const { data: invoicesData, isLoading } = useQuery({
  queryKey: computed(() => ['invoices', 'sale', selectedYear.value, selectedMonth.value, page.value, search.value]),
  queryFn: async () => {
...
<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-3xl font-bold text-gray-900">{{ t('invoices.salesTitle') }}</h1>
      <p class="mt-2 text-gray-600">{{ t('invoices.salesSubtitle') }}</p>
    </div>

    <Card>
      <CardHeader>
        <div class="flex items-center justify-between flex-wrap gap-3">
          <CardTitle>{{ t('invoices.salesTitle') }}</CardTitle>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ t('invoices.tableCustomer') }}</th>
                ...
                <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">{{ t('invoices.tableTax') }}</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              >
                ...
                <td class="px-6 py-4 text-sm text-gray-700">{{ invoice.customer_name ?? '-' }}</td>
                ...
                <td class="px-6 py-4 text-sm text-gray-500 text-right">{{ formatCurrency(invoice.tax_amount) }}</td>
